...
PYEOF
  ...
  #
  # This checks registration, which is necessary and not sufficient: a hook can
  # be wired correctly and still decline to act on every call. Whether each of
  # these would actually fire is NOT checked here, so the label says so rather
  # than reading as a clean bill of health. The section below covers the one
  # class of dead gate that is externally visible — a file test written into a
...
for h in want:
    ...
    if not hits:
        ...
        # "registered" only. Whether this hook would act on any given call is
        # decided inside the script and is not visible from here, so the label
        # does not claim more than it checked.
