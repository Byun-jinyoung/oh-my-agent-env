...
write_managed_block() {
  ...
  command -v python3 &>/dev/null || { { printf '%s\n' "$OMA_BLOCK_BEGIN"; cat "$body"; printf '%s\n' "$OMA_BLOCK_END"; } > "$target"; return; }
...
# "Ours" = the command references a script file this repo ships, plus any name
# the manifest declares retired. Basename alone is not enough: $CONFIG_DIR/hooks
# also holds foreign hooks (context-mode-cache-heal.mjs, gsd-*.js) that we must
# never touch. Retired names are needed because a hook deleted from the repo
# stops matching "ships" and its stale entry would otherwise survive forever.
owned = {p.name for p in repo_hooks.glob("*.js")} | {p.name for p in repo_hooks.glob("*.mjs")}
...
def is_ours(group):
    ...
    for entry in group.get("hooks", []):
        ...
        if str(hooks_dir) in cmd and any(name in cmd for name in owned):
            return True
    return False
