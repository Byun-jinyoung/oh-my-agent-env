...
# Replace the oh-my-agent-env-managed region of a file with fresh content,
# preserving everything the user wrote outside the markers.
#   $1 = target file   $2 = file holding the generated body
# Before this existed, assembly was `{ ... } > "$target"` — a truncating write
# over ~/.claude/CLAUDE.md, ~/.codex/AGENTS.md and ~/.gemini/GEMINI.md, so any
# personal note added to a CLI's global memory was destroyed on the next sync
# with no backup. A target that predates the markers is backed up once, then
# adopted, so the old content stays recoverable instead of vanishing.
OMA_BLOCK_BEGIN="<!-- oh-my-agent-env:begin — generated by setup.sh sync; edits inside are overwritten -->"
OMA_BLOCK_END="<!-- oh-my-agent-env:end -->"
# Exit status: 0 the target now carries the current block (freshly written, or
# already byte-identical); 3 the target was left exactly as it was and is
# therefore STALE. Every branch returning 3 refuses on purpose — a shredded
# global memory file is unrecoverable while a stale one is not — but the
# refusal has to be audible. While all of them returned 0, a sync printed
# success with one CLI still serving rules the other two had already retired,
# and nothing anywhere said so.
write_managed_block() {
  local target="$1" body="$2" rc
  # Without python3 the only fallback available in pure shell is a truncating
  # redirect, which is exactly the data loss this function exists to prevent.
  # Leaving the file stale is recoverable; overwriting it is not.
  if ! command -v python3 &>/dev/null; then
    log_and_print "    [SKIP] python3 missing — $target left unchanged (would truncate)"
    return 3
  fi
  python3 - "$target" "$body" "$OMA_BLOCK_BEGIN" "$OMA_BLOCK_END" << 'PYEOF' | sed 's/^/    /'
import os, shutil, sys, tempfile, time
from pathlib import Path

target, body_path, begin, end = Path(sys.argv[1]), Path(sys.argv[2]), sys.argv[3], sys.argv[4]
body_text = body_path.read_text(encoding="utf-8").rstrip()

# A body containing either marker would make the NEXT run split in the wrong
# place and shred the file. Refuse instead: leaving the target stale is
# recoverable, silently corrupting a CLI's global memory file is not.
if begin in body_text or end in body_text:
    print(f"[WARN] generated body contains a managed-block marker — {target.name} left unchanged")
    sys.exit(3)
...
# Exactly one marker means the file was hand-edited or half-written. Treating
# that as "no markers" would move the visible content into a backup and leave
# the live file holding only the generated block. Refuse and let a human look.
if (begin in old) != (end in old):
    print(f"[WARN] {target.name} has a broken managed block (one marker only) — left unchanged")
    sys.exit(3)
...
if begin in old and end in old:
    ...
    if old.strip():
        bak = target.with_name(f"{target.name}.bak.{int(time.time())}")
        shutil.copyfile(target, bak)
        print(f"[BACKUP] {bak.name} (pre-marker content preserved)")
    new = block
...
try:
    ...
    raise
PYEOF
  # The body is piped through sed to indent it, so `$?` here is sed's status —
  # always 0 — and reading it would discard every refusal python just reported.
  rc=${PIPESTATUS[0]}
  return "$rc"
}
...
assemble_global_rules() {
  ...
  local cli dir tools target stale=""
  for cli in claude codex antigravity; do
    ...
    if write_managed_block "$target" "$dir/.oma-assembled.tmp"; then
      log_and_print "    [OK] $target (Layer A rules/ + $cli tools)"
    else
      log_and_print "    [STALE] $target NOT updated — $cli still serves the previous rules"
      stale="$stale $cli"
    fi
    rm -f "$dir/.oma-assembled.tmp"
  done
  # Skipping one CLI while the other two advance is the whole failure class:
  # the harness assembles one rule set for three targets, so a partial sync
  # leaves them disagreeing about what the rules are. Reported, not silent.
  [ -z "$stale" ] || {
    log_and_print "    [FAIL] rules diverged across CLIs — not updated:$stale"
    return 1
  }
}
...
# symlink step and the settings.json registration step. Falls back to a glob
# that excludes test fixtures if the manifest or python3 is unavailable, so a
# degraded environment still installs hooks rather than silently installing none.
hook_manifest_scripts() {
...
' "$manifest" 2>/dev/null && return 0
  ...
  local f
  for f in "$SCRIPT_DIR/runtimes/claude/hooks/"*.js; do
    [ -f "$f" ] || continue
...
# hand-edited entry converges instead of lingering forever. Entries pointing at
# anything we do not own (rtk, gsd-*, context-mode-cache-heal.mjs, user-added)
# are never inspected or moved. Idempotent: a second run is a byte-for-byte
# no-op and writes nothing. The hook SCRIPTS are symlinked by cmd_sync's
# [2] Claude hooks step, which reads the same manifest.
ensure_rules_enforcement_hooks() {
  ...
  python3 - "$CONFIG_DIR" "$SCRIPT_DIR/runtimes/claude/hooks" << 'PYEOF' | sed 's/^/    /'
import json, os, shutil, sys, tempfile
from pathlib import Path

config_dir = Path(sys.argv[1])
repo_hooks = Path(sys.argv[2])
settings = config_dir / "settings.json"
hooks_dir = config_dir / "hooks"
manifest_path = repo_hooks / "manifest.json"
...
#
# The match is on the full path `<hooks_dir>/<script>`, not on the directory and
# the basename independently. Those two conditions checked separately make
#   node "<hooks_dir>/foreign-wrapper.mjs" --note bash-size-guard.js
# look like ours and get deleted: it sits in the dir and mentions our basename,
# while invoking something else entirely. $CONFIG_DIR/hooks is shared with
# foreign hooks (context-mode-cache-heal.mjs, gsd-*.js) that we must never touch.
owned = {h["script"] for h in want if isinstance(h, dict) and "script" in h}
owned |= {str(name) for name in manifest.get("retired", [])}
owned_paths = {f"{hooks_dir}/{name}" for name in owned}
...
def is_ours(group):
    ...
    for entry in group.get("hooks", []):
        ...
        if any(path in cmd for path in owned_paths):
            return True
    return False
...
before = json.dumps(data, indent=2, ensure_ascii=False, sort_keys=True)

hooks = data.setdefault("hooks", {})
if not isinstance(hooks, dict):
    print("[WARN] settings.json 'hooks' is not an object — skipping")
    sys.exit(0)

# 1. Drop every group we own; foreign groups keep their position.
for event, arr in list(hooks.items()):
    if not isinstance(arr, list):
        continue
    hooks[event] = [g for g in arr if not is_ours(g)]

# 2. Re-add from the manifest, in manifest order.
missing = []
for item in want:
    event, matcher, script = item["event"], item.get("matcher"), item["script"]
...
if missing:
    print(f"[WARN] manifest lists missing script(s): {', '.join(missing)}")

if before == after:
    print(f"[OK] rules-enforcement hooks reconciled ({len(want)} in manifest, no change)")
else:
    # Backup holds the immediately-previous state, refreshed on every real
    # change. A write-once backup goes stale after the first sync and then
    # restores a version from months ago; timestamped copies accumulate forever.
    bak = settings.parent / (settings.name + ".bak.rules-enforcement")
    if settings.exists():
        shutil.copyfile(settings, bak)
        print(f"[BACKUP] {bak.name}")
    # Atomic: a crash or full disk mid-write must not leave settings.json
    # truncated — Claude Code would then start with no hooks at all.
    ...
    try:
        ...
        tmp.unlink(missing_ok=True)
        raise
    print(f"[OK] rules-enforcement hooks reconciled ({len(want)} in manifest, settings.json updated)")
PYEOF
}
