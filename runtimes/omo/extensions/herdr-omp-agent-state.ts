// VENDORED by oh-my-agent-env. Source of truth for OMO's herdr agent-state
// reporting. Deployed to ~/.omo/agent/extensions/herdr-omp-agent-state.ts by
// lib/sync/agent-clis.sh (sync_omo_herdr_extension).
//
// Why vendored and not `herdr integration install omp`: herdr's omp integration
// installs into ~/.omp/agent/extensions and refuses when that dir is absent, but
// OMO's real agent dir is ~/.omo/agent (senpi configDir ".omo"). OMO shares
// senpi/pi-tui's extension API, so this hook loads and reports state 1:1.
//
// To refresh against a newer herdr: `mkdir -p ~/.omp/agent/extensions &&
// herdr integration install omp`, then re-vendor the generated file (bump the
// HERDR_INTEGRATION_VERSION marker below), re-add this banner, and re-apply the
// two `agent: "omp"` -> `agent: "omo"` edits (search "oma: report label").
//
// Original herdr provenance (do not edit body; add custom hooks beside it):
...
function reportSession(sessionStartSource = "startup"): Promise<void> {
  ...
  return sendRequest({
    ...
    params: {
      pane_id: paneId,
      source,
      // oma: report label "omo" (not herdr's default "omp"). The agent KIND stays
      // omp (argv[0]=omp detection); this only sets the sidebar display label so
      // OMO shows as "omo", not "omp". herdr accepts an arbitrary --agent label.
      agent: "omo",
      seq: nextReportSeq(),
